...
Chart.prototype.drawCoordinates = function()
{
	...
	// Horizontal
	for(var h = 20; h <= this.height -20; h += 40)
	{
		this.drawLine(40, h, this.width -40, h);		
	}

	// Vertical
	for(var w = this.width - 40; w >= 40; w -= 80)
	{
		this.drawLine(w, 20, w, this.height -20);		
	}
	
};

Chart.prototype.drawPunkte = function()
{		
	var kreise = document.getElementsByTagNameNS("http://www.w3.org/2000/svg", "circle");
	
	var i = kreise.length;
	
	while(i--)
	{
		document.getElementById("chartSVG").removeChild(kreise[i]);
	}
		
	
	var self = this;
	
	self.drawPunkt(1560, 60, true);
	
	this.data.forEach(function(element)
	{	
		self.drawPunkt(element.x, element.y, false);
	});
...
var ChartPoint = {
		...
		moveElement : function( event )
		{
			...
			if(event.offsetY > 25 && event.offsetY < 175)
			{
		...
		deselectElement : function( event )
		{
			...
			if(this.selectedElement)
			{
			    ...
			    chart.data.push({y:cy, x:chart.zero});
