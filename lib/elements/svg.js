...
{	
	...
	console.log( JSON.stringify( this.model.data.punkte ));
	
	var punkteById = this.model.data.punkte;
	

	punkteById.sort( function( a, b )
	{
	...
	while(lines--)
	{
		...
		{
			if(this.model.data.punkte[lines].x < this.maxX && this.model.data.punkte[lines].x < ( this.model.data.punkte[lines -1].x + this.model.msProTag * 3) )
			{
	...
	{			
		...
		{
			if( this.model.data.punkte[dots].id == id)
			{
				this.drawPunkt( this.model.data.punkte[dots], false, true);									
			}
			else
			{
				this.drawPunkt( this.model.data.punkte[dots], false, false);													
			}
		}
	}
	
	if( this.model.data.punkte[0].id == id)
	{
		var punkt = {};
		
		punkt.x = this.maxXRealtime;
		punkt.y = this.model.data.punkte[0].y;
		punkt.id = id;
	...
	{
		this.drawPunkt( { x:this.maxXRealtime, y:this.maxY, id:id}, true, active);			
	}	
