...
{	
	...
	while(dots--)
	{				
		var punkt1 = Object.create( punkteById[dots] );
	...
	var p = { x: this.maxXRealtime, id : id };
	var y = this.model.getDefaultForId( id );
	
	p.y = (punkt2 && punkt2.id == id) ? punkt2.y : y;

	this.drawPunkt( p, true, true);	
...
{	
	...
	var farbe = this.getFarbwertForXY(current.id);
...
{			
	...
	if(!active)
	{
		kreis.setAttribute("opacity", "0.5");
	}
